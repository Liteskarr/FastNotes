...
class GroupItemWidget(QWidget):
    ...
    def _configure_ui(self):
        ...
        self.title.editingFinished.connect(self._handle_name_changing)
    ...
    def _swap_mode(self) -> None:
        """
        Меняет режим редактирования имение группы.
        :return:
        """
    ...
    def _selection_filter(self) -> None:
        """
        Проверяет, возможно ли проводить действия с виджетом.
        """
        if not self.edit_checker.checkState():
            self.title.deselect()

    def _handle_name_changing(self) -> None:
        """
        Обрабатывает изменение имени группы.
        """
